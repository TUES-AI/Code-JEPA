...
#SBATCH --job-name=finetune-unixcoder-v2
...
#SBATCH --mem=64G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/Code-JEPA/logs/finetune-unixcoder-v2.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/Code-JEPA/logs/finetune-unixcoder-v2.%j.err
...
CHECKPOINT=${PROJECT_DIR}/runs/pretrain-unixcoder-v2/latest.pt
BENCH_ROOT=/valhalla/projects/bg-eng-01/scratch/vvasilev/code-jepa-small/benchmarks/codexglue
OUTPUT_DIR=${PROJECT_DIR}/runs/finetune-unixcoder-v2
...
echo "=== Fine-tuning UniXcoder (v2) on POJ-104 (2 GPUs) ==="
torchrun --nproc_per_node=2 --master_port=29610 scripts/finetune_clone_benchmarks.py \
    --benchmark poj104 \
...
echo "=== Fine-tuning UniXcoder (v2) on BigCloneBench (2 GPUs) ==="
torchrun --nproc_per_node=2 --master_port=29610 scripts/finetune_clone_benchmarks.py \
...
echo "UniXcoder (v2) fine-tune done. Results:"
