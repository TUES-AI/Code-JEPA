...
#SBATCH --job-name=finetune-codejepa-rw015
...
#SBATCH --mem=64G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/Code-JEPA/logs/finetune-codejepa-rw015.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/Code-JEPA/logs/finetune-codejepa-rw015.%j.err
...
CHECKPOINT=${PROJECT_DIR}/runs/pretrain-codejepa-rw015/latest.pt
BENCH_ROOT=/valhalla/projects/bg-eng-01/scratch/vvasilev/code-jepa-small/benchmarks/codexglue
OUTPUT_DIR=${PROJECT_DIR}/runs/finetune-codejepa-rw015
...
echo "=== Fine-tuning Code-JEPA (rank_weight=0.15) on POJ-104 (2 GPUs) ==="
torchrun --nproc_per_node=2 --master_port=29612 scripts/finetune_clone_benchmarks.py \
    --benchmark poj104 \
...
echo "=== Fine-tuning Code-JEPA (rank_weight=0.15) on BigCloneBench (2 GPUs) ==="
torchrun --nproc_per_node=2 --master_port=29612 scripts/finetune_clone_benchmarks.py \
...
echo "Code-JEPA (rank_weight=0.15) fine-tune done. Results:"
