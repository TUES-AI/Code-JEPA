...
#SBATCH --job-name=finetune-codejepa
...
#SBATCH --mem=64G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/Code-JEPA/logs/finetune-codejepa.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/Code-JEPA/logs/finetune-codejepa.%j.err
...
CHECKPOINT=${PROJECT_DIR}/runs/pretrain-codejepa/latest.pt
BENCH_ROOT=/valhalla/projects/bg-eng-01/scratch/vvasilev/code-jepa-small/benchmarks/codexglue
OUTPUT_DIR=${PROJECT_DIR}/runs/finetune-codejepa
...
echo "=== Fine-tuning Code-JEPA on POJ-104 (2 GPUs) ==="
torchrun --nproc_per_node=2 --master_port=29601 scripts/finetune_clone_benchmarks.py \
    --benchmark poj104 \
...
echo "=== Fine-tuning Code-JEPA on BigCloneBench (2 GPUs) ==="
torchrun --nproc_per_node=2 --master_port=29601 scripts/finetune_clone_benchmarks.py \
...
echo "Code-JEPA fine-tune done. Results:"
